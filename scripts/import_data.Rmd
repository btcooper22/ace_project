...
```{python Read old data}
ace_data_old_unprocessed = pd.read_csv("data/ace_data_extra.csv")

ace_data_old = (ace_data_old_unprocessed
            .pipe(run_default_pipeline))
            
ace_data_old.to_csv("data/ace_data_old.csv")
```

```{python Read new data}
ace_data_new_unprocessed = pd.read_csv("data/brand_new_data.csv")

ace_data_new = (ace_data_new_unprocessed
            .pipe(run_default_pipeline))
            
ace_data_new.to_csv("data/ace_data_new.csv")
quit
```

```{r Load and check}
# Load data
old_df <- read_csv("data/ace_data_old.csv")
new_df <- read_csv("data/ace_data_new.csv")
...
with(results, table(dataset, hospital_reqd))
with(results, table(dataset, referral_from))
with(results, table(dataset, referral_profession))
with(results, table(dataset, age))
with(results, table(dataset, address))
with(results, table(dataset, gender))
with(results, table(dataset, referral_date))
with(results, table(dataset, referral_time))
with(results, table(dataset, illness_severity))
with(results, table(dataset, activity_level))
with(results, table(dataset, gut_feeling))
with(results, table(dataset, ox_sat))
with(results, table(dataset, resp_rate))
with(results, table(dataset, heart_rate))
with(results, table(dataset, temp))
with(results, table(dataset, sepsis))
with(results, table(dataset, safeguarding))
with(results, table(dataset, food_allergy))
with(results, table(dataset, drug_allergy))
with(results, table(dataset, other_allergy))
with(results, table(dataset, group_ethnicity))
with(results, table(dataset, ox_sat_low))
with(results, table(dataset, age_range))
with(results, table(dataset, ace_resp_rate_cat))
with(results, table(dataset, ace_heart_rate_cat))
with(results, table(dataset, apls_resp_rate_cat))
with(results, table(dataset, apls_heart_rate_cat))
with(results, table(dataset, meets_ace_criteria))
with(results, table(dataset, mentions_asthma))
with(results, table(dataset, mentions_salbutamol))

# Write
results %>% 
    write_csv("data/ace_data_april21.csv")
```
